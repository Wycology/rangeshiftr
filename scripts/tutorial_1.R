...
dir.create(paste0(dirpath,'Inputs'), showWarnings = TRUE) # Creating folder for input files
...
# Plot land cover map and highlight all cells with initial species distribution - option 1:

plot(UKmap, col = brewer.pal(n = 6, name = "Spectral"), axes = F)

plot(rasterToPolygons(SpDist, dissolve = F), add = T)

# Plot land cove map and highlight cells with initial species distribution - option 2 with categorical legend:
